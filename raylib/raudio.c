...
//----------------------------------------------------------------------------------
    ...
    #define AUDIO_DEVICE_SAMPLE_RATE              44100    // Device output channels: stereo
#endif
...
void InitAudioDevice(void)
{
    ...
    TRACELOG(LOG_INFO, "    > Sample rate:   %d -> %d", AUDIO_DEVICE_SAMPLE_RATE, AUDIO.System.device.playback.internalSampleRate);
...
Music LoadMusicStream(const char *fileName)
{
    ...
    else if (IsFileExtension(fileName, ".xm"))
    {
        ...
        if (result == 0)    // XM AUDIO.System.context created successfully
        {
            ...
            music.stream = InitAudioStream(AUDIO_DEVICE_SAMPLE_RATE, bits, AUDIO_DEVICE_CHANNELS);
...
Music LoadMusicStreamFromMemory(const char *fileType, unsigned char* data, int dataSize)
{
    ...
    {
        jar_xm_context_t *ctxXm = NULL;
        int result = jar_xm_create_context_safe(&ctxXm, (const char*)data, dataSize, AUDIO_DEVICE_SAMPLE_RATE);
        if (result == 0)    // XM AUDIO.System.context created successfully
        {
            music.ctxType = MUSIC_MODULE_XM;
            jar_xm_set_max_loop_count(ctxXm, 0);    // Set infinite number of loops
            ...
            // NOTE: Only stereo is supported for XM
            music.stream = InitAudioStream(AUDIO_DEVICE_SAMPLE_RATE, bits, 2);
            music.sampleCount = (unsigned int)jar_xm_get_remaining_samples(ctxXm)*2;    // 2 channels
            music.looping = true;   // Looping enabled by default
            jar_xm_reset(ctxXm);   // make sure we start at the beginning of the song
    ...
    else if (TextIsEqual(fileExtLower, ".mod"))
    {
        ...
        if (result > 0)
        {
            ...
            // NOTE: Only stereo is supported for MOD
            music.stream = InitAudioStream(AUDIO_DEVICE_SAMPLE_RATE, 16, 2);
            music.sampleCount = (unsigned int)jar_mod_max_samples(ctxMod)*2;    // 2 channels
            music.looping = true;   // Looping enabled by default
            musicLoaded = true;
...
int GetAudioStreamBufferSizeDefault()
{
    ...
    if (AUDIO.Buffer.defaultSize == 0)
        AUDIO.Buffer.defaultSize = AUDIO_DEVICE_SAMPLE_RATE/30;
